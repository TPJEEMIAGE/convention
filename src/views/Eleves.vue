<template>
  <div class="eleves">
    <div>
      <label>Eleve</label>
      <vue-infinite-autocomplete :data-source="elevesOptions" @select="handleSelectEleve"></vue-infinite-autocomplete>
    </div>
    <div v-if="selectedEleve != null">
      <label>Période</label>
      <select v-model="selectedPeriodeId">
        <option v-for="periode in periodes" :key="periode.idPeriode" :value="periode.idPeriode">{{periode.dateDebut}} - {{periode.dateFin}}</option>
      </select>
    </div>
    <div v-if="selectedEleve != null">
      <label>Entreprise</label>
      <vue-infinite-autocomplete :data-source="entrepriseOptions" @select="handleSelectEntre"></vue-infinite-autocomplete>
      <div>
        <span>Vous ne trouvez pas votre entreprise ?</span><input type="button" value="Ajouter"/>
      </div>
    </div>
    <div v-if="selectedEntreprise != null">
      <label>Responsable</label>
      <vue-infinite-autocomplete :data-source="responsableOptions" @select="handleSelectResp"></vue-infinite-autocomplete>
      <div>
        <span>Vous ne trouvez pas le responsable ?</span><input type="button" value="Ajouter"/>
      </div>
    </div>
...
export default {
  ...
  methods:{
    ...
    createStage(){
      ...
      console.log(stage)
      axios.post('http://localhost/convention/createStage.php', stage).then(response => {
  ...
  mounted(){
    console.log("launch")
    axios.get('http://localhost/convention/getElevesList.php',
            ).then(response => {
  ...
  watch:{
    eleveId:function(){
      ...
      axios.get('http://localhost/convention/getPeriodes.php',
          {params :{
            codeSection: this.selectedEleve.codeSection
          }}
            ).then(response => {
      ...
      axios.get('http://localhost/convention/getEntrepriseList.php',
            ).then(response => {
                this.entrepriseData = response.data;
                this.entrepriseOptions = [];
                this.entrepriseData.forEach(element => {
                  let option = {
                    text: element.raisonSociale + " - " + element.adresse + " " + element.adresse2,
    ...
    entrepriseId:function(){
      ...
      axios.get('http://localhost/convention/getRespList.php', {params :{
