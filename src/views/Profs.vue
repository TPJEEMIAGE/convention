  ...
  <div class="profs">
    <div>
      ...
      <select v-model="selectedSection">
        <option v-for="section in listSection" :key="section.codeSection" :value="section.codeSection">{{section.codeSection}}</option>
      </select>
    </div>
    <div v-if="selectedSection != null">
      <label>Référent : </label>
      <select v-model="selectedProf" >
        <option v-for="refs in listRefs" :key="refs.idProfesseur" :value="refs.idProfesseur">{{refs.nom}} {{refs.prenom}}</option>
      </select>
    </div>
    <div v-if="selectedProf != null">
      <label>Elève : </label>
      <select v-model="selectedStageId" >
        <option v-for="stage in stageList" :key="stage.idStage" :value="stage.idStage">{{stage.nomEleve}} {{stage.prenomEleve}}</option>
      </select>
    </div>
...
export default {
  ...
  mounted(){
      axios.get('http://localhost/convention/getSectionList.php',
            ).then(response => {
  ...
  watch: {
    selectedSection : function() {
      ...
      axios.get('http://localhost/convention/getRefList.php',{
    ...
    selectedProf : function(){
      this.selectedStageId = null
      this.selectedStage = null
      axios.get('http://localhost/convention/getStageList.php',{
